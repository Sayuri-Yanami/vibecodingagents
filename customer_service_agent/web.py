...
class CustomerServiceHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            result = self.agent.ask(question, history, miss_count)
...
def main() -> int:
    ...
    CustomerServiceHandler.agent = CustomerServiceAgent(settings, index)
    CustomerServiceHandler.settings_status = {
        "status": "ok",
    ...
    print(
        f"AI 客服已启动：http://{settings.web_host}:{settings.web_port}\n"
        "按 Ctrl+C 停止服务。"
    )
    try:
